...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT id, written FROM orders WHERE id = :id");
    ...
    $stmt = $pdo->prepare("UPDATE orders SET written = 1 WHERE id = :id");
    ...
    if ($affectedRows > 0) {
        ...
        respond(false, 'Заказ не был обновлен');
    }
} catch (\PDOException $e) {
    error_log('Database error in writtenOrders: ' . $e->getMessage());
    respond(false, 'Ошибка при обновлении заказа: ' . $e->getMessage());
}
