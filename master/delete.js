/**
 * Отправляет запрос на бэкенд для удаления записи
 ...
 */
export async function deleteRecord(id, type) {
    try {
        ...
        if (type === 'orders' || type === 'order') {
            endpoint = '/api/orders/deleteOrder/';
            paramName = 'orderId';
            entityName = 'заказа';
        } else if (type === 'feedback' || type === 'messages') {
            endpoint = '/api/feedback/deleteFeedback/';
        ...
        console.log(`Отправка запроса на удаление ${endpoint} с ${paramName}:`, id);
        ...
        return {
            success: false,
            message: `Ошибка сервера при удалении ${entityName}: ` + error.message
        };
    }
}
